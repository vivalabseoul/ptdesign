  ...
  useEffect(() => {
    ...
    const timeTracker = setInterval(() => {
      setElapsedTime((prev) => {
        ...
        // 3분 기준으로 안내 메시지 변경
        if (seconds >= 180) {
          setMessage("분석이 거의 완료되었습니다. 잠시만 더 기다려주세요...");
        } else if (seconds >= 120) {
          setMessage("AI가 상세 보고서를 작성하고 있습니다...");
        } else if (seconds >= 90) {
          setMessage("접근성 및 SEO 분석 중...");
        } else if (seconds >= 60) {
          setMessage("모바일 경험 및 체류 유도 요소 분석 중...");
        } else if (seconds >= 30) {
          setMessage("첫인상 및 이탈 방지 요소 분석 중...");
        } else if (seconds >= 10) {
    ...
    // 완료되지 않았으면 95%까지만 천천히 증가 (약 3분 기준)
    const progressTimer = setInterval(() => {
      setProgress((prev) => {
        ...
        // 3분(180초) 동안 95%까지 도달하도록 설정
        // 500ms 간격으로 360번 = 180초, 95/360 ≈ 0.26% per interval
        const increment = Math.random() * 0.15 + 0.18;
