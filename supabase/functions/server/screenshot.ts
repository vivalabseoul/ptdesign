...
export async function takeScreenshot(url: string): Promise<string | null> {
  try {
    ...
    if (screenshotApiKey) {
      ...
      const screenshotUrl = `https://shot.screenshotapi.net/screenshot?token=${screenshotApiKey}&url=${encodeURIComponent(url)}&width=1920&height=1080&output=image&file_type=png&wait_for_event=load`;
    ...
    if (htmlcsstoimageApiKey) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          console.error(`HTML/CSS to Image API 오류 (${response.status}):`, errorText);
        }
      } catch (error) {
    ...
    if (urlboxApiKey && urlboxSecret) {
      ...
      const screenshotUrl = `https://api.urlbox.io/v1/${urlboxApiKey}/png?url=${encodeURIComponent(url)}&width=1920&height=1080&wait_for=load`;
    ...
    console.warn("스크린샷 API 키가 설정되지 않았습니다. 다음 환경 변수 중 하나를 설정하세요:");
    ...
    console.warn("참고: 스크린샷 생성은 선택적 기능이며, API 키가 없어도 분석은 정상적으로 진행됩니다.");
    return null;
  } catch (error) {
    console.error("스크린샷 생성 중 예외 발생:", error instanceof Error ? error.message : String(error));
    return null;
  }
}
 ...
 * Base64로 스크린샷 다운로드
 */
export async function downloadScreenshotAsBase64(imageUrl: string): Promise<string | null> {
  try {
