...
const { evaluationCriteria, improvements } = SAMPLE_REPORT_DATA;
...
const scoreHistoryData = [
  { month: "5개월 전", score: 65 },
  { month: "4개월 전", score: 68 },
  { month: "3개월 전", score: 72 },
  { month: "2개월 전", score: 75 },
  { month: "1개월 전", score: 79 },
  { month: "현재", score: 82 },
];
...
export function AnalysisReport() {
  ...
  useEffect(() => {
    async function loadReport() {
      if (!id) return;
      try {
        // 샘플 프로젝트인 경우 직접 데이터 사용 (데이터 불일치 방지)
        if (id === SAMPLE_PROJECT_ID) {
          setReportData({
            ...SAMPLE_REPORT_DATA,
            url: "https://www.naver.com",
            analyzedAt: new Date(Date.now() - 86400000).toLocaleDateString(),
  ...
  // Use destructured variables directly as fallbacks
  const currentEvaluationCriteria: EvaluationCriteria[] =
    reportData?.evaluationCriteria || evaluationCriteria;
  const currentImprovements: Improvement[] =
  ...
  const report: ReportSummary = reportData
    ? {
        ...reportData,
  ...
  // 레이더 차트 데이터
  const radarData = evaluationCriteria.map((c) => ({
  ...
  const categoryData = [
    ...
    {
      category: "첫인상",
      count: improvements.filter((i) => i.category.includes("첫인상")).length,
      color: "#84A98C",
    },
    {
      category: "이탈 방지",
      count: improvements.filter((i) => i.category.includes("이탈")).length,
      color: "#F59E0B",
    },
    {
      category: "모바일 경험",
      count: improvements.filter((i) => i.category.includes("모바일")).length,
      color: "#84A98C",
    },
    {
      category: "체류 유도",
      count: improvements.filter((i) => i.category.includes("체류")).length,
      color: "#293241",
    },
    {
      category: "접근성",
      count: improvements.filter((i) => i.category.includes("접근성")).length,
      color: "#6B7280",
    },
    {
      category: "SEO",
      count: improvements.filter((i) => i.category.includes("SEO")).length,
      color: "#10B981",
    },
  ];
  ...
  const priorityData = [
    {
      priority: "치명적",
      count: improvements.filter((i) => i.priority.toLowerCase() === "critical")
        .length,
      color: "#DC2626",
    },
    {
      priority: "높음",
      count: improvements.filter((i) => i.priority.toLowerCase() === "high")
        .length,
      color: "#EE6C4D",
    },
    {
      priority: "중간",
      count: improvements.filter((i) => i.priority.toLowerCase() === "medium")
        .length,
      color: "#F59E0B",
    },
    {
      priority: "낮음",
      count: improvements.filter((i) => i.priority.toLowerCase() === "low")
        .length,
      color: "#84A98C",
    },
  ];
  ...
  const scoreHistory = reportData
    ? generateScoreHistory(totalScore)
    : scoreHistoryData;
                ...
                >
                  <div className="flex items-center justify-between mb-6">
                    <h3
                      ...
                      style={{ color: "var(--primary)" }}
                    >
                      개선 필요 항목 ({improvements.length}개)
                    </h3>
                    <button
                      onClick={() => setActiveTab("improvements")}
                      className="text-base font-medium text-gray-500 hover:text-gray-900 flex items-center gap-1"
